...
namespace cpp_server
{
    ...
    Error TritonEngine::readModelConfig()
    {
        ...
        if (client_config.protocol == ProtocolType::HTTP)
        {
            ...
            if (!tc_err.IsOk())
            {
                return Error(Error::Code::INTERNAL, "Failed to get triton model metadata");
            }

            tc_err = tc::ParseJson(&model_metadata_json, model_metadata);
            if (!tc_err.IsOk())
            {
                return Error(Error::Code::INTERNAL, "Failed to parse triton model metadata");
            }
            ...
            if (!tc_err.IsOk())
            {
                return Error(Error::Code::INTERNAL, "Failed to get triton model config");
            }

            tc_err = tc::ParseJson(&model_config_json, model_config_str);
            if (!tc_err.IsOk())
            {
                return Error(Error::Code::INTERNAL, "Failed to parse triton model config");
            }
            if (!ParseModelHttp(
                    model_metadata_json, model_config_json, batch_size, &model_config))
            {
                return Error(Error::Code::INTERNAL, "Failed to parse model configuration and metadata");
            }
        }
        else
        {
    ...
    Error TritonEngine::initializeMemory()
    {
        ...
        if (!tc_err.IsOk())
        {
            return Error(Error::Code::INTERNAL, "Unable to get TritonClient::Input input");
        }
        ...
        if (!tc_err.IsOk())
        {
            return Error(Error::Code::INTERNAL, "Unable to get TritonClient::Output output");
        }
    ...
    Error TritonEngine::validate(const std::vector<InferenceData<uint8_t>> &data)
    {
        if (data.size() != batch_size)
        {
            return Error(Error::Code::VALIDATION_ERROR, "Number of data is different from batch size.");
        }
        ...
        {
            return Error(Error::Code::VALIDATION_ERROR, "Total data bytesize is different from allocated bytesize.");
        }

        return Error::Success;
    }

    Error TritonEngine::postprocess(const std::unique_ptr<tc::InferResult> &result, InferenceResult<uint8_t> &res, const size_t &batch_size, const std::string &output_name)
    {
        if (!result->RequestStatus().IsOk())
        {
            return Error(Error::Code::INFERENCE_ERROR, result->RequestStatus().Message());
        }
        ...
        if (!err.IsOk())
        {
            return Error(Error::Code::INTERNAL, "Unable to get shape for output " + output_name);
        }
    ...
    Error TritonEngine::process(const std::vector<InferenceData<uint8_t>> &infer_data, std::vector<InferenceResult<uint8_t>> &infer_results)
    {
        Error p_err;
        ...
        while (!last_request)
        {
            ...
            if (!tc_err.IsOk())
            {
                return Error(Error::Code::INFERENCE_ERROR, "Failed resetting input ptr");
            }
            ...
            for (int idx = 0; idx < batch_size; ++idx)
            {
                ...
                if (!tc_err.IsOk())
                {
                    return Error(Error::Code::INFERENCE_ERROR, "Failed setting input");
                }
            ...
            if (!tc_err.IsOk())
            {
                return Error(Error::Code::INFERENCE_ERROR, "Failed sending synchronous infer request");
            }
